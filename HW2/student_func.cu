                      ...
                      unsigned char* const blueChannel)
{
  ...
  uchar4 input_pixel = inputImageRGBA[thread_1D_pos];
  redChannel[  thread_1D_pos] = input_pixel.x;
  greenChannel[thread_1D_pos] = input_pixel.y;
  blueChannel[ thread_1D_pos] = input_pixel.z;
}
                                ...
                                const float* const h_filter, const size_t filterWidth)
{
  ...
  int num_bytes_filter = sizeof(float) * filterWidth * filterWidth;
  checkCudaErrors(cudaMalloc(&d_filter, num_bytes_filter));

  //Copy the filter on the host (h_filter) to the memory you just allocated
  //on the GPU.  cudaMemcpy(dst, src, numBytes, cudaMemcpyHostToDevice);
  //Remember to use checkCudaErrors!
  checkCudaErrors(cudaMemcpy(d_filter, h_filter, num_bytes_filter, cudaMemcpyHostToDevice));
}
                        ...
                        const int filterWidth)
{
  ...
  const dim3 blockSize(32, 32);  // 32^2=1024
